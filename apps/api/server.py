...
def _cors_origins() -> list[str]:
    raw = os.getenv("FSBI_CORS_ORIGINS") or os.getenv("CORS_ORIGINS") or "http://localhost:5173,http://127.0.0.1:5173"
...
def _load_local_food_hint(query: str) -> dict[str, str] | None:
    path = Path(__file__).resolve().parent / "data" / "fragrance_food_sources.csv"
    if not path.exists():
        return None
    try:
        df = pd.read_csv(path)
